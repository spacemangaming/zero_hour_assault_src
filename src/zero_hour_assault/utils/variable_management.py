...
def randomstr(min,max): return str(random.randint(min,max))
...
def string_split(the_string, the_delimiter, require_full_delimiter):
    if the_delimiter == "\r\n":
        the_delimiter = "\n"
        ...
        if event.type == pygame.KEYDOWN:
            if event.unicode and event.unicode != "\r\n":
                if (
                    ...
                    and event.key != pygame.K_LCTRL
                    and event.key != pygame.K_RCTRL
                    and ord(event.unicode)>=32
                ):
                    return event.unicode.replace(chr(22),"",-1)
    return ""
