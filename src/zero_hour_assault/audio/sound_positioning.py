...
def position_sound_1d(handle, listener_x, source_x, pan_step, volume_step):
    position_sound_custom_1d(
        handle, listener_x, source_x, pan_step, volume_step, 0.0, 0.0
    )
...
def position_sound_custom_1d(
    ...
    handle.set_3dposition(delta, 0, 0)
...
def position_sound_custom_2d(
    ...
    if theta > 0.0:
        rotational_source_x = (
            dummy(math.cos(theta) * (source_x - listener_x))
            - (math.sin(theta) * (source_y - listener_y))
            + listener_x
        )
        rotational_source_y = (
            dummy(math.sin(theta) * (source_x - listener_x))
    ...
    handle.set_3dposition(source_x, source_y, 0)
...
def position_sound_custom_3d(
    ...
    listener_x = round(listener_x)
    listener_y = round(listener_y)
    ...
    if theta > 0.0:
        ...
        source_x = dummy(rotational_source_x)
        source_y = dummy(rotational_source_y)
    # Next, we calculate the delta between the listener and the source.
    delta_x = source_x - listener_x
    final_pan += delta_x * pan_step
    final_volume -= delta_x * volume_step
    delta_y = source_y - listener_y
    final_volume -= delta_y * volume_step
    if listener_y > source_y:
        final_pitch -= abs(behind_pitch_decrease)
    if listener_z > source_z:
        final_pitch -= abs(behind_pitch_decrease)
	...
	if not keep_pitch:
		if handle.pitch != final_pitch:
			handle.pitch = final_pitch
	"""
    handle.set_3dposition(delta_x, delta_y, delta_z)
    if not keep_pitch:
        if handle.pitch != final_pitch:
            handle.pitch = final_pitch
