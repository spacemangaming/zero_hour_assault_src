...
half_down = 135
streight_down = 180
def move(x, y, z, deg, zdeg, dir, zdir):
	if dir!=0:
		d2=deg+dir
		if d2>=360:
			d2-=360
	...
	ztheta=calculate_theta(zdeg)
	r=vector(0, 0, 0)
	r.x=x+math.sin(theta)
	r.y=y+math.cos(theta)
	r.z=z+math.sin(ztheta)
	return r
...
def get_3d_distance(x1, y1, z1, x2, y2, z2):
	return abs(math.sqrt((round(x2) - round(x1)) ** 2 + (round(y2) - round(y1)) ** 2 + (round(z2) - round(z1)) ** 2))
def calculate_x_y_angle(x1, y1, x2, y2, deg=0):
	x = round(x2) - round(x1)
	y = round(y2) - round(y1)
	if x == 0 and y == 0:
		return 0
...
def rotate(source_x, source_y, listener_x, listener_y, theta):
	r = vector()
	r.x=((math.cos(theta) * (source_x - listener_x))-(math.sin(theta) * (source_y - listener_y))+listener_x)
	r.y = (		(math.sin(theta) * (source_x - listener_x))		+ (math.cos(theta) * (source_y - listener_y))		+ listener_y)
	return r
def find_appropriate_facing(orig_x, orig_y, target_x, target_y):
	delta_x = target_x - orig_x
	delta_y = orig_y - target_y	
