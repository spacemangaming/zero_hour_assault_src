...
def timebombloop():
	for t in g.timebombs:
		if t.beeptimer.elapsed>=t.beeptime:
			t.beeptimer.restart()
			g.play("timebombbeep",t.x,t.y,t.z,t.map)
#			for p in g.players:
#				if get_3d_distance(t.x,t.y,t.z,p.x,p.y,p.z)<=t.range and t.map==p.map:
#					g.n.send_reliable(p.peer_id,"distsound timebombbeep "+str(t.x)+" "+str(t.y)+" "+str(t.z)+" "+str(t.map),0)

		if t.checktimer.elapsed>=1000:
			t.checktimer.restart()
			if not file_exists("maps/"+t.map+".map"):
				g.timebombs.remove(t)
				return
		if t.explodetimer.elapsed>=t.explodetime:
			...
			for i in range(len(g.npcs)):
				...
				if g.npcs[i].map==t.map and get_3d_distance(t.x, t.y, t.z, g.npcs[i].x, g.npcs[i].y, g.npcs[i].z)<=t.range and g.npcs[i].map==t.map and g.npcs[i].health>0:
					...
					g.play("timebombhit",g.npcs[i].x,g.npcs[i].y,g.npcs[i].z,g.npcs[i].map,g.npcs[i])
			...
			for i in range(len(g.zombies)):
				...
				if g.zombies[i].map==t.map and get_3d_distance(t.x, t.y, t.z, g.zombies[i].x, g.zombies[i].y, g.zombies[i].z)<=t.range and g.zombies[i].map==t.map and g.zombies[i].health>0:
					...
					g.zombies[i].mode=1
					g.play("timebombhit",g.zombies[i].x,g.zombies[i].y,g.zombies[i].z,g.zombies[i].map,g.zombies[i])
			...
			g.timebombs.remove(t)
			return
def place_timebomb(x,y,z,map,owner):
	place=timebomb(x,y,z,map,owner)
	g.timebombs.append(place)
